...
confirmBtn.addEventListener('click', async () => {
  // Kumpulin item yang dipilih
  const items = [];
  Object.values(tickets).forEach(t => {
    const qty = Number(document.getElementById('qty-' + t.id).value || 0);
    if (qty > 0) items.push({ tiket_id: t.id, qty });
  });
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.message || 'Checkout gagal. Coba lagi.');
    }

    const data = await res.json();
    window.location.href = data.redirect || "{{ route('histories.index') }}";
